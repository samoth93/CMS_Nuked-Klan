...
function add_sondage() {
    ...
    echo "</table><div style=\"text-align: center;\"><br /><input type=\"submit\" value=\"" . _ADDTHISPOLL . "\" /></div>\n"
       . "<div style=\"text-align: center;\"><br />[ <a href=\"index.php?file=Survey&amp;page=admin\"><b>" . _BACK . "</b></a> ]</div></form><br /></div></div>\n";
}

function send_sondage($titre, $option, $groupsAccess) {
    global $nuked, $user;

    $time = time();
    $titre = mysql_real_escape_string(stripslashes($titre));

    $groupsAccess = implode('|', array_keys($groupsAccess));

    $sql = mysql_query("INSERT INTO " . SURVEY_TABLE . " ( `sid` , `titre` , `date` , `groups_access` ) VALUES ( '' , '" . $titre . "' , '" . $time . "' , '" . $groupsAccess . "' )");
...
function edit_sondage($poll_id) {
    ...
    $sql = mysql_query("SELECT titre, groups_access FROM " . SURVEY_TABLE . " WHERE sid = '" . $poll_id . "'");
    list($titre, $groupsAccess) = mysql_fetch_array($sql);
   ...
   echo "<div class=\"content-box\">\n" //<!-- Start Content Box -->
    ...
    groupsCheckbox($groupsAccess);

    echo "</table><div style=\"text-align: center;\"><br /><input type=\"hidden\" name=\"poll_id\" value=\"".$poll_id."\" /><input type=\"submit\" value=\"" . _MODIFTHISPOLL . "\" /></div>\n"
       . "<div style=\"text-align: center;\"><br />[ <a href=\"index.php?file=Survey&amp;page=admin\"><b>" . _BACK . "</b></a> ]</div></form><br /></div></div>\n";
}

function modif_sondage($poll_id, $titre, $option, $newoption, $groupsAccess) {
    global $nuked, $user;
    ...
    $sql = mysql_query("UPDATE " . SURVEY_TABLE . " SET titre = '" . $titre . "' , groups_access = '" . $groupsAccess . "' WHERE sid = '" . $poll_id . "'");
...
function main(){
   ...
   $dbsGroups = 'SELECT id, nameGroup
    ...
    $sql = mysql_query('SELECT sid, titre, date, groups_access FROM ' . SURVEY_TABLE . ' ORDER BY sid DESC');
    $count = mysql_num_rows($sql);
    while (list($poll_id, $titre, $date, $groupsAccess) = mysql_fetch_array($sql)) {
        $date = nkDate($date);
        $titre = printSecuTags($titre);

        $groupsAccess = explode('|', $groupsAccess);
...
?>
                <div class="iconGroupList adminIcon icon-groups">
                    <ul>
<?php
                        if (is_array($groupsAccess)) {
                            foreach ($groupsAccess as $groupId) {
...
switch ($_REQUEST['op']) {
    ...
    case 'send_sondage':
        send_sondage($_REQUEST['titre'], $_REQUEST['option'], $_REQUEST['groups']);
        break;

    case 'modif_sondage':
        modif_sondage($_REQUEST['poll_id'], $_REQUEST['titre'], $_REQUEST['option'], $_REQUEST['newoption'], $_REQUEST['groups']);
        break;
