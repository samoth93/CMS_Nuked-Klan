...
function update_sondage($poll_id, $voteID) {
    global $nuked, $userIp, $user, $theme, $bgcolor2, $bgcolor3;
    ...
    if (!empty($voteID) && is_numeric($voteID) && is_numeric($poll_id)) {
        if (verif_check($poll_id) == 0) {
            $sql = mysql_query('SELECT groups_access FROM ' . SURVEY_TABLE . ' WHERE sid = ' . $poll_id);
            list($groupsAccess) = mysql_fetch_array($sql);
            ...
            if (nkHasVisitor() === false) {
                $arrayGroupsUser = $GLOBALS['user']['idGroups'];
            }
...
function affich_res($poll_id) {
    ...
    if(!empty($poll_id) && is_numeric($poll_id)) {
        ...
        if ($active == 1 && nkAccessModule('Comment') && nkIsModEnabled('Comment')) {
            echo '<tr><td>';
